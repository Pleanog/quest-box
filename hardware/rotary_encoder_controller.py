...
class RotaryEncoderController:
    def __init__(self, name, event_queue, clk_pin, dt_pin, button_pin, options=None):
        self.name = name
        ...
        while self.running:
            ...
            if clk_current_state != self.clk_last_state:
                ...
                if GPIO.input(self.dt_pin) != clk_current_state:
                    ...
                    direction = f"clockwise - index {self.current_index}"
                else:
                    self.current_index = (self.current_index - 1) % len(self.options)
                    direction = f"counter_clockwise - index {self.current_index}"
