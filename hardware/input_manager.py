...
# # Define your options here or load them from a separate config file
...
# class InputManager:
#     def __init__(self, event_queue, bus_lock):
...
#         # We will now create the RotaryEncoderController instances directly
#         # and manage them within this class.
#         self.encoder_controllers = {}
        
#         # Create a list to store polling functions
#         self.polling_functions = []
        
#         # Mapping device types to their handler functions
#         self.handlers = {
...
#         print(f"Configured SX1509 button '{color}' on IO{pin}")
...
#         # Decide which options list to pass based on the encoder name
#         options = []
#         if name == "image_dial":
...
#         # Start the worker thread for I2C devices
#         self.worker_thread = Thread(target=self.poll_devices, daemon=True)
#         self.worker_thread.start()
        
#         # Start all rotary encoder controllers
#         for controller in self.encoder_controllers.values():
#             controller.start()
...
#         # Stop all rotary encoder controllers
#         for controller in self.encoder_controllers.values():
#             controller.stop()
...
#                 check_func()
#                 time.sleep(0.05) # Small delay to prevent I2C bus overload
        
# # Example usage for testing
# if __name__ == "__main__":
#     q = queue.Queue()
#     bus_lock = Lock()
#     im = InputManager(q, bus_lock)

#     # Define all devices
#     # distance-sensor uses pins 23, 24
#     # LEDs use pin 18
#     # vibration motor uses 17
#     device_configs = [
...
#         # {"type": "rotary_encoder", "name": "number_dial", "clk_pin": 12, "dt_pin": 13, "button_pin": 26},
#     ]
# #     button 36 16
# #  clk 38 20
# # dt 40 21
...
#         im.add_device(config)

#     im.start()
#     print("Listening for input events. Press Ctrl+C to exit.")
#     try:
#         while True:
#             # Process events from the queue if needed
#             if not q.empty():
#                 event = q.get()
